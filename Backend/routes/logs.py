...
def get_logs():
    ...
    pagination = q.order_by(AuditLog.timestamp.desc()).paginate(page=page, per_page=50, error_out=False)
    return jsonify({
...
def log_stats():
    ...
    return jsonify({
        "total": q.count(),
        "warnings": q.filter_by(severity="warning").count(),
        "critical": q.filter_by(severity="critical").count(),
        "today": q.filter(AuditLog.timestamp >= today).count()
    }), 200
